...
logic  [2:0] set_ink_r, set_ink_g, set_ink_b, set_offtint;
...
wire [1:0] joy_p54;
wire [3:0] joy_do_sgb;
reg [14:0] sgb_lcd_data;
wire [15:0] sgb_border_pix;
reg sgb_lcd_clkena, sgb_lcd_on, sgb_lcd_vsync;
wire sgb_lcd_freeze;
...
// DMG only - the SGB is gone. Two things it did remain needed here:
// the joypad matrix (gb.v reads joy_din from it even without SGB), and the
// two-stage ce-clocked register on the LCD signals, kept so the video timing
// stays identical to the upstream core.
wire p14 = joy_p54[0];
wire p15 = joy_p54[1];
wire [3:0] joy_dir     = ~{ joystick_0[2], joystick_0[3], joystick_0[1], joystick_0[0] } | {4{p14}};
wire [3:0] joy_buttons = ~{ joystick_0[7], joystick_0[6], joystick_0[5], joystick_0[4] } | {4{p15}};
assign joy_do_sgb = joy_dir & joy_buttons;

reg [14:0] lcd_data_d1;
reg [1:0]  lcd_mode_d1;
reg        lcd_clkena_d1, lcd_on_d1, lcd_vsync_d1;

always @(posedge clk_sys) begin
  if (ce_cpu) begin
    lcd_data_d1    <= lcd_data;
    lcd_mode_d1    <= lcd_mode;
    lcd_clkena_d1  <= lcd_clkena;
    lcd_on_d1      <= lcd_on;
    lcd_vsync_d1   <= lcd_vsync;

    sgb_lcd_data   <= lcd_data_d1;
    sgb_lcd_mode   <= lcd_mode_d1;
    sgb_lcd_clkena <= lcd_clkena_d1;
    sgb_lcd_on     <= lcd_on_d1;
    sgb_lcd_vsync  <= lcd_vsync_d1;
  end
end

assign sgb_lcd_freeze = 1'b0;
